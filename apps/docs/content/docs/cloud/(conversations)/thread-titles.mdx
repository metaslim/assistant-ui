...
| Model | The provider's first listed model | A model the chosen provider lists. A model the provider does not list is refused at save time. | The model that writes the title. |
